...
date: 'July 25, 2022'
dateUpdated: 'July 29, 2022'
description: Finding the most efficient approach to build a library of components
tags: ['UI Library']
---
...
So, on the next project, I decided to use a well-known and well-tested library (like [Material UI](https://mui.com/)), and it was great! Adding some content and a few styles and my components were ready to use. But not everything was shiny:
...
In the last years, we have seen that unstyled libraries of components got some attraction, like [Radix UI](https://www.radix-ui.com/), [Reach UI](https://reach.tech/) or [Headless UI](https://headlessui.com/). So, why not use them instead of our custom components?
...
On the other side, a stream of new templates (full sections ready to use) started to get some attraction. In this case, the raw HTML + CSS code is provided without any abstraction and the full section is ready to copy-paste into your project. I'm talking about collections like [Tailwind UI](https://tailwindui.com/) or [Flowbite](https://flowbite.com/).

All the previous solutions had something in common: we created a system of components, from the most basic to the most complex. If we keep in mind the [Atomic Design](https://bradfrost.com/blog/post/atomic-web-design/) methodology, we started from the atoms, then to the molecules, organisms, templates, and finally the pages. The core idea of creating a system of components was implicit in the previous solutions, following a Bottom-Up approach.
...
In the end, the core idea is to create a place to [share, dare and care](https://www.valtech.com/career/our-values/), a hub that we use to help ourselves in our projects and where we contribute back with more sections, with our leanings, and with our experiences.

## Appendix A: Comparison of existing solutions analysed

| Library              | Flowbite (There is a new major release in August 2022)                                                                | Tailwind UI                                                                           |
| -------------------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| URL                  | https://flowbite-react.com/                                                                                           | https://tailwindui.com/                                                               |
| Has React components | Yes                                                                                                                   | No, they provide the HTML + CSS, we need to create React components out of them.      |
| Accesibility         | Not the best. Probably we end up refactoring their components or creating new components from libraries like Radix UI | To review, probably we need to build the components from unstyled UI libraries anyway |
| Theming              | There is a theme provided, but probably the theme and the components need adjustments                                 | No, we need to build it                                                               |
| Figma                | They provide a figma file                                                                                             | No figma file                                                                         |
| Commerce sections    | Yes                                                                                                                   | Yes                                                                                   |
